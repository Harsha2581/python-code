    ...
    if request.method == 'POST':
        if request.form['sub'] == 'Submit':
            ...
            try:
                con = sqlite3.connect('mydatabase.db')
                cursorObj = con.cursor()
                cursorObj.execute(f"SELECT Name from Users WHERE Email='{email}';")
...
def dashboard():
    global name
    gas, fire, tilt, sos, temp, humidity, bpm, buzzer = readFirebase()
...
def get_realtime_data():
    gas, fire, tilt, sos, temp, humidity, bpm, buzzer = readFirebase()
    pollution = predict_pollution_level(temp,humidity,gas)
    pushbullet_noti('Pollution Level',pollution)
    return jsonify({
        'gas': gas,
        'fire': fire,
        'tilt': tilt,
